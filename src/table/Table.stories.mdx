...
<Meta title='Componentreact/Table' />
...
export const Template = (args) => {
  useEffect(() => {
    ...
    styleEle.textContent = `
        ...
        `;
    tabBox.shadowRoot.appendChild(styleEle);
  }, [args]);
  return (
    <div>
      <wheat-table />{' '}
...
69 行代码 实现一个 san(百度自研框架)、vue、react 都可以用的 table 组件

基于 webcomponents 封装一个通用组件库! 打破框架的隔阂.

看着这么多 react 优秀的组件库，我们要想办法用上

### 技术实现

antd 等等这些组件库的组件功能都比较全。应该充分利用上。毕竟都是 js、html、css 组成的。处理好隔离就能很好的使用。

![实现思路](https://user-images.githubusercontent.com/24740506/216220940-3486c8d2-2472-48ae-a098-b9bbb26d37c6.png)
...
| render                            | 生成复杂数据的渲染函数，参数分别为当前行的值，当前行数据，行索引                                                                                                                             | function(text, record, index) => [RenderComponent](#rendercomponent)                                                                                                                | -                      |                                |
...
| 参数          | 说明                   | 类型       | 默认值 |
| ------------- | ---------------------- | ---------- | ------ |
| componentName | antd 中组件名称        | string     | null   |
| type          | 元素类型（div、span）  | string     | div    |
| ...other      | 相关组件或者元素的配置 | [key]: any |        |
